...
from model.reid import load_reid_model, extract_reid_features

from .basetrack import BaseTrack, TrackState

class STrack(BaseTrack):

    def __init__(self, tlwh, max_n_features=100, from_det=True):
...
class OnlineTracker(object):
    ...
    def __init__(self, min_ap_dist=0.64, max_time_lost=30, use_tracking=True, use_refind=True):
    ...
    def update(self, image, tlwhs):
        ...
        detections = [STrack(tlwh, from_det=True) for tlwh in tlwhs]
        pred_dets = []
        if self.use_tracking:
            tracks = [STrack(t.self_tracking(image), from_det=False)
                      for t in itertools.chain(self.tracked_stracks, self.lost_stracks)
                      if t.is_activated and t.tracklet_score() > 0.6]

            pred_dets.extend(tracks)
        ...
        # prediction
        r_tracked_stracks = [tracked_stracks[i] for i in u_track]
        dists = matching.iou_distance(r_tracked_stracks, pred_dets)
        matches, u_track, u_pred = matching.linear_assignment(dists, thresh=0.7)
        for itracked, idet in matches:
            r_tracked_stracks[itracked].update(pred_dets[idet], self.frame_id, image, update_feature=True)
        for it in u_track:
            track = r_tracked_stracks[it]
            track.mark_lost()
            lost_stracks.append(track)
        ...
        out_lost_stracks = [t for i, t in enumerate(self.lost_stracks)
                            if (self.frame_id - t.end_frame <=4 and t.is_activated)]
